...
Test test_a0_task2_problems_numerical("a0.task2.problems.numerical", []() {
    ...
    int factor = 3;
    // need float here or cast to int
    auto helper = [&](int x, int y) { return x < (y * 1.0 / factor); };
...
Test test_a0_task2_problems_vector("a0.task2.problems.vector", []() {
    ...
    // not end()(not included), last is end()-1
    auto it = one_to_ten.end() - 1;
    int last_element = *it;
    ...
    for (size_t i = 0; i < vec1.size(); i++) {
        for (size_t j = 0; j < vec2.size(); j++) {
            for (size_t k = 0; k < vec3.size(); k++) {
                ...
                // check each pair, not all three. result of (==) is boolean(0 for false)
                if ((vec1.at(i) == vec2.at(j))&&(vec1.at(i) == vec3.at(k))
                    && (vec3.at(k) == vec2.at(j))) count++;
            }
        }
    }
