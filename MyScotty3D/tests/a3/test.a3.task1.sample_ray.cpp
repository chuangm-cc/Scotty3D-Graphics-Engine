...
Test test_a3_task1_sample_ray_more_pixels("a3.task1.sample_ray.more_pixels", []() {
 // Create a camera and get the transform matrix from camera to world
 auto [cam, iV] = setup_cam(Vec2(5, 5), Vec3(0, 0, -1), Vec3(), Degrees(2.0f * std::atan(0.5f)), 1.0f);
 ...
 RNG rng;
 for (uint32_t i = 0; i < N; i++) {
  auto [ret, pdf] = cam.sample_ray(rng, 4, 4);
  ret.transform(iV);
