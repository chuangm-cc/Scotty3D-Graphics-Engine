...
struct Jittered{
	Jittered(Vec2 size = Vec2(1.0f)) : size(size) {
	}
	Vec2 size;
	int index;
	// needs number of samples
	Vec2 sample(RNG &rng,int num_sum) const;
};

struct MultiJittered{
	MultiJittered(Vec2 size = Vec2(1.0f)) : size(size) {
	}
	Vec2 size;
	int index;
	// needs number of samples
