...
Trace Triangle::hit(const Ray& ray) const {
	...
	// use functions in vec3.h!
	// inline Vec3
	float equ_denominator = dot(cross(e1,d),e2);
	// If the denominator 0
	if(equ_denominator==0){
		return ret;
	}
	float u = - dot(cross(s,e2),d) / equ_denominator;
	float v = dot(cross(e1,d),s) / equ_denominator;
	float t = - dot(cross(s,e2),e1) / equ_denominator;
	float w = 1.0f - u - v;
	...
	// Don't forget that the intersection point on the ray should be within the ray's dist_bounds
    ...
    ret.position = ray.point + d*t; // where was the intersection?
    ret.normal = (w*v_0.normal + u*v_1.normal + v*v_2.normal).unit();   // what was the surface normal at the intersection?
                           // (this should be interpolated between the three vertex normals)
