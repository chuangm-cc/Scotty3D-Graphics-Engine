...
Spectrum Pathtracer::sample_direct_lighting_task4(RNG &rng, const Shading_Info& hit) {
	...
	auto in_dir = hit.object_to_world.rotate(scatter.direction);
	//TODO: construct a ray travelling in that direction
	// NOTE: because we want emitted light only, can use depth = 0 for the ray
	uint32_t depth = 0;
	Vec2 dist_bounds = Vec2{EPS_F, std::numeric_limits<float>::infinity()};
	Ray in_ray(hit.pos, in_dir, dist_bounds, depth);
	//TODO: trace() the ray to get the emitted light (first part of the return value)
	Spectrum emitted_light = trace(rng, in_ray).first;
	...
	if(hit.bsdf.is_specular()){
		Spectrum weight =  scatter.attenuation;
    	radiance += emitted_light * weight;/// in_dir.y;
		return radiance;
	}
	float bsdf_pdf = hit.bsdf.pdf(hit.out_dir, scatter.direction);
    Spectrum weight =  scatter.attenuation/bsdf_pdf;
    radiance += emitted_light * weight;
...
Spectrum Pathtracer::sample_direct_lighting_task6(RNG &rng, const Shading_Info& hit) {
	...
	// If the BSDF is discrete (hit.bsdf.is_specular()), we don't need to bother sampling lights
	if(hit.bsdf.is_specular()){
		return sample_direct_lighting_task4(rng,hit);
	}
	// randomly choose whether we get our sample from Material::scatter or Pathtracer::sample_area_lights. 
	// Choose between the strategies with equal probability.
	bool use_area = rng.coin_flip(1/2.f);

	Vec3 ray_dir;
	auto scatter = hit.bsdf.scatter(rng, hit.out_dir, hit.uv);
	if(use_area){
		ray_dir = sample_area_lights(rng,hit.pos);
	}else{
		ray_dir = scatter.direction;
		ray_dir = hit.object_to_world.rotate(ray_dir);
	}
	Vec2 dist_bounds = Vec2(EPS_F, std::numeric_limits<float>::infinity());
	// Create a new world-space ray (the "shadow ray")
	// so the ray depth can be zero.
	Ray world_ray = Ray(hit.pos,ray_dir,dist_bounds,0);
	// call Pathtracer::trace to get incoming light
	// modify Ray::dist_bounds,ray depth can be zero.
	auto incoming_light = trace(rng,world_ray).first;

	//use Material::evaluate, Material::pdf, and Pathtracer::area_lights_pdf to compute the proper weighting
	// float bsdf_pdf = hit.bsdf.pdf(hit.out_dir, in_dir); Spectrum weight =  scatter.attenuation/bsdf_pdf;

	float bsdf_pdf = hit.bsdf.pdf(hit.out_dir,scatter.direction);

	// Pathtracer::area_lights_pdf takes a world-space position and direction,
	float area_pdf = area_lights_pdf(hit.pos,ray_dir);

	// float weight_bsdf = bsdf_pdf/(bsdf_pdf+area_pdf);
	// float weight_area = area_pdf/(bsdf_pdf+area_pdf);
	float avg_pdf = (area_pdf+bsdf_pdf)/2.f;


	// auto bsdf_a = scatter.attenuation;
	auto bsdf_a = hit.bsdf.evaluate(hit.out_dir,hit.world_to_object.rotate(world_ray.dir),hit.uv); // same

	radiance += incoming_light*bsdf_a/avg_pdf;
...
Spectrum Pathtracer::sample_indirect_lighting(RNG &rng, const Shading_Info& hit) {
	...
	// Ray::dist_bounds so that the ray does not intersect at time = 0
	// reduce the ray depth
	uint32_t depth = hit.depth - 1;
	Vec2 dist_bounds = Vec2{EPS_F, std::numeric_limits<float>::infinity()};
	// Vec3 point, Vec3 dir,Vec2 dist_bounds uint32_t depth = 0
	Ray in_ray = Ray(hit.pos, in_dir, dist_bounds ,depth);
	//TODO: trace() the ray to get the reflected light (the second part of the return value)
	auto reflected_light = trace(rng,in_ray).second;
	//TODO: weight properly depending on the probability of the sampled scattering direction and set radiance

	// update here:!
	if(hit.bsdf.is_specular()){
		Spectrum weight =  scatter.attenuation;
    	reflected_light = reflected_light *weight ;/// in_dir.y;
		Spectrum radiance = reflected_light;
		return radiance;
	}
	float bsdf_pdf = hit.bsdf.pdf(hit.out_dir, scatter.direction);
	Spectrum weight =  scatter.attenuation/bsdf_pdf;
...
std::pair<Spectrum, Spectrum> Pathtracer::trace(RNG &rng, const Ray& ray) {
	...
	if(log_rng.coin_flip(0.0005f)) log_ray(ray, 10.0f);
