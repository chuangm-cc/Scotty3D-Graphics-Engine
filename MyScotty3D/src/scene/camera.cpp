...
std::pair<Ray, float> Camera::sample_ray(RNG &rng, uint32_t px, uint32_t py) {
	...
	(void)sensor_pixel;
	// sensor_pixel.transform(camera_to_world);
	// vertical_fov
	// vertical_fov indicates the vertical field of view of the camera (in degrees, not radians)
	float vertical_fov_r = (vertical_fov*1.0f/2) * float(M_PI) / 180;
	float ver_ratio = tan(vertical_fov_r) * 2; //
	...
	float w_ratio = aspect_ratio * ver_ratio;
	// std::cout<<w_ratio;

	// film.width and film.height are parameters giving the camera's sensor size in pixels.
	// You can think of this as the sizes of the images you want to render
	Vec3 map_pos = Vec3(0,0,-1);
	// between 0-1 ? float
	// std::cout<<sensor_pixel.x<<std::endl;
	//  sensor pixel location (0,0) maps to (-w/2,-h/2,-1),
	//  and sensor pixel location (film.width,film.height) maps to (w/2,h/2,-1).
	// divide by film.width
    map_pos.x = sensor_pixel.x/film.width * w_ratio - w_ratio/2.0f;
    map_pos.y = sensor_pixel.y/film.height * ver_ratio - ver_ratio/2.0f;

	// ray.dir = sensor_position - ray.point; // Compute direction from origin to sensor position
    // ray.dir.normalize();

	//Build ray:
	Ray ray;
	ray.point = Vec3(); //ray should start at the origin
	ray.dir = map_pos; //TODO: compute from sensor plane position
