...
Mat4 Transform::local_to_world() const {
	...
	if (std::shared_ptr< Transform > parent_ = parent.lock()) {
		...
		res = parent_->local_to_world() * res * local_to_parent();
		// std::shared_ptr< Transform > cur = parent_;
		// while(std::shared_ptr< Transform > parent_new_ = cur->parent.lock()){
		// 	res = parent_new_->local_to_parent()*res ;
		// 	cur = parent_new_;
		// }
	} else {
		//case where transform doesn't have a parent
		res= res * local_to_parent();
	}
...
Mat4 Transform::world_to_local() const {
	...
	if (std::shared_ptr< Transform > parent_ = parent.lock()) {
		//case where transform has a parent
		res = res * parent_to_local()*parent_->world_to_local();
	} else {
		//case where transform doesn't have a parent
		res= res*parent_to_local();
	}
	return res; //<-- wrong, but here so code will compile
}
