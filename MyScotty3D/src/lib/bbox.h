...
// >>>>>>> 241811acf480ebc484205dafee6bc0e2e09947ae

		float tmin,tmax;
		if(r.x==0){
			tmin = min.x - o.x>0?FLT_MAX:-FLT_MAX;
			tmax = max.x - o.x>=0?FLT_MAX:-FLT_MAX;
		}else{
			tmin = (min.x - o.x) / r.x; 
			tmax = (max.x - o.x) / r.x; 
		}
		...
		if(r.y==0){
			tymin = min.y - o.y>0?FLT_MAX:-FLT_MAX;
			tymax = max.y - o.y>=0?FLT_MAX:-FLT_MAX;
		}else{
			tymin = (min.y - o.y) / r.y; 
			tymax = (max.y - o.y) / r.y; 
		}
		...
		float tzmin,tzmax;
		if(r.z==0){
			tzmin = min.z - o.z>0?FLT_MAX:-FLT_MAX;
			tzmax = max.z - o.z>=0?FLT_MAX:-FLT_MAX;
		}else{
			tzmin = (min.z - o.z) / r.z; 
			tzmax = (max.z - o.z) / r.z;
		} 
