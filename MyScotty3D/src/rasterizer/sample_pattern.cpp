...
std::vector<SamplePattern> const& SamplePattern::all_patterns() {
	...
	auto CreateYourOwnSamplePattern = []() {
		...
		float mid_distance = distance/3;
		// This will cause it to segfault when used, so be sure to change it!
		std::vector<Vec3> centers_and_weights = {};
		centers_and_weights.reserve(h*3 + 2*4);
		float weight = 1.0f / (5*3 + 2*4);
		for (uint32_t x = 0; x < 3; ++x) {
			for (uint32_t y = 0; y < 5; ++y) {
				// centers_and_weights.emplace_back((x + 0.5f) / w, (y + 0.5f) / h, weight);
				centers_and_weights.emplace_back(half_distance + x*2*distance, 
				half_distance + y*distance, weight);
			}
		}
		...
		for (uint32_t x = 0; x < 2; ++x) {
			for (uint32_t y = 0; y < 4; ++y) {
				// centers_and_weights.emplace_back((x + 0.5f) / w, (y + 0.5f) / h, weight);
				centers_and_weights.emplace_back(mid_distance*(x+1), 
				(y+1)*distance, weight);
			}
		}
