...
Framebuffer::Framebuffer(uint32_t width_, uint32_t height_, SamplePattern const& sample_pattern_)
	...
	pattern_size((uint32_t)sample_pattern_.centers_and_weights.size()){
...
HDR_Image Framebuffer::resolve_colors() const {
	...
	for (uint32_t y = 0; y < height; ++y) {
		for (uint32_t x = 0; x < width; ++x) {
				...
				Spectrum color_one = Spectrum();
				for (uint32_t s = 0; s < pattern_size; ++s) {
					// float dis_x = sample_pattern.centers_and_weights[s].x;
					// float dis_y = sample_pattern.centers_and_weights[s].y;
					color_one += color_at(x, y, s) * sample_pattern.centers_and_weights[s].z;
				}
				image.at(x, y) = color_one;
		}
	}
