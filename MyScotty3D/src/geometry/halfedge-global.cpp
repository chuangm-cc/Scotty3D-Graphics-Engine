...
void Halfedge_Mesh::triangulate() {
	//A2G1: triangulation
	// int count = 0;
	for(Halfedge_Mesh::FaceRef f = faces.begin(); f != faces.end(); f++){
		if(f->boundary) continue;
		// std::cout<<"a";
		uint32_t line_num = f->degree() -3;
		...
		auto h_before_last =h0;
		do{
			h_before_last = h_before_last ->next;
		}while(h_before_last ->next->next!=h0);

		// connect vertix
		auto end_v = h_last->vertex;
		...
		auto last_h2 = h_last;
		auto f_last = h0->face;
		auto last_last_h2 = h_last;


		// std::cout<<f->id<<std::endl;
		// std::cout<<h0->id<<std::endl;
		// std::cout<<h0->next->id<<std::endl;
		// std::cout<<h0->next->next->id<<std::endl;
		auto last_f = f;
		for(uint32_t i= 0;i<line_num;i++){
			// collect
			auto v_start = h_it->vertex;

			auto new_e = emplace_edge();
			auto new_h1 = emplace_halfedge();
			auto new_h2 = emplace_halfedge();
			auto new_f = emplace_face();

			// connect
			h_before->next = new_h1;
			new_e->halfedge = new_h2;
			new_f->halfedge = new_h2;
			// HalfedgeRef twin_, HalfedgeRef next_, VertexRef vertex_, EdgeRef edge_, FaceRef face_
			new_h1 -> set_tnvef(new_h2,last_h2,v_start,new_e,f_last);
			new_h2 -> set_tnvef(new_h1,h_it,end_v,new_e,new_f);
		...
		h_before_last->next = last_last_h2;
		h_before_last ->face = last_f;
	}
...
void Halfedge_Mesh::linear_subdivide() {
	...
	//(TODO)
	for(Halfedge_Mesh::VertexRef v = vertices.begin(); v != vertices.end(); v++){
...
void Halfedge_Mesh::catmark_subdivide() {
	...
	for(Halfedge_Mesh::EdgeRef e= edges.begin(); e != edges.end(); e++){
		...
		if(!e->halfedge->face->boundary){
			af1 = face_vertex_positions[e->halfedge->face];
			n+=1;
		}
		if(!e->halfedge->twin->face->boundary){
			af2 = face_vertex_positions[e->halfedge->twin->face];
			n+=1;
		}
		edge_vertex_positions[e] = (me*2.0f+af1+af2)/n;
	}
	...
	for(Halfedge_Mesh::VertexRef v= vertices.begin(); v != vertices.end(); v++){
		...
		do{	
			if(!h_it->face->boundary){
				F_sum+=face_vertex_positions[h_it->face];
				// R_sum+=edge_vertex_positions[h_it->edge];	
				count2+=1;
			}
			R_sum += h_it->edge->center();
			F_count+=1;
		...
		Vec3 R = R_sum/F_count;
		Vec3 F = F_sum/F_count;
		
		// std::cout<<F<<R<<v->position;
		Vec3 res;
		if(v->on_boundary()){
			// res= F+R+abs(F_count-3)*(v->position);
			auto a = v->halfedge-> twin->vertex->position;
			auto c = v->halfedge->twin->next->twin->vertex->position;
			vertex_positions[v] = 1.0f/8.0f *a + 1.0f/8.0f *c + 3.0f/4.0f * v->position;
		}else{
			res= F+2.0f*R+(F_count-3)*(v->position);
			res = res/F_count;

			// std::cout<<res.x;

			vertex_positions[v] = res;
		}
...
bool Halfedge_Mesh::loop_subdivide() {
	...
	for(Halfedge_Mesh::VertexRef v= vertices.begin(); v != vertices.end(); v++){
		...
		if(v->on_boundary()){
			Vec3 v_new = Vec3();
			v_new = v->position*3.0f/(2*2);
			vertex_new_pos[v] = v_new;
			// std::cout<<n<<std::endl;
			continue;
		}
		Vec3 v_new = Vec3();
		// use helper func -> sum = ave*n
		v_new = (1.0f-n*u)*v->position*1.0f + u*1.0f*(v->neighborhood_center()*n);
		vertex_new_pos[v] = v_new;
		// std::cout<<v_new.x<<v_new.y<<v_new.z<<std::endl;
	}
    ...
    for(Halfedge_Mesh::EdgeRef e= edges.begin(); e != edges.end(); e++){
		if(e->on_boundary()){
			Vec3 v_new = e->center();
			edge_new_pos[e] = v_new;
			n++;
			continue;
		}
		Vec3 a = e->halfedge->vertex->position;
		Vec3 b = e->halfedge->twin->vertex->position;
		Vec3 c= e->halfedge->next->twin->vertex->position;
		Vec3 d = e->halfedge->twin->next->twin->vertex->position;
		Vec3 v_new = (3.0f/8.0f) *(a+b) + (1.0f/8.0f)*(c+d);
		n++;
		edge_new_pos[e] = v_new;
	}
	// std::cout<<edges.size();
	...
	for (uint32_t i = 0; i < (uint32_t)n; i++) {
		...
		if(edge_new_pos.count(e)){
			VertexRef v_new= split_edge(e).value();

			v_new->position = edge_new_pos[e];

			// std::cout<<"a"<<std::endl;
			if(!e->on_boundary()){
				// auto e0 = e->halfedge->vertex->position;
				// auto e1 = e->halfedge->twin->vertex->position;
				// std::cout<<e0.x<<e0.y<<e1.z<<std::endl;
				// std::cout<<e1.x<<e1.y<<e1.z<<std::endl;

				auto t1=e->halfedge;
				auto h1=t1->twin;
				new_edges.push_back(t1->next->next->edge);
				new_edges.push_back(h1->next->edge);

				auto e0 = t1->next->next->edge->halfedge->vertex->position;
				auto e1 = t1->next->next->edge->halfedge->twin->vertex->position;
				// std::cout<<e0.x<<e0.y<<e0.z<<std::endl;
				// std::cout<<e1.x<<e1.y<<e1.z<<std::endl;
			}else{
				auto h1=e->halfedge;
				new_edges.push_back(h1->next->edge);
				auto e0 = h1->next->edge->halfedge->vertex->position;
				auto e1 = h1->next->edge->halfedge->twin->vertex->position;
				// std::cout<<e0.x<<e0.y<<e0.z<<std::endl;
				// std::cout<<e1.x<<e1.y<<e1.z<<std::endl;
			}
			...
			// do{
			// 	if(it!=t1)
			// 		new_edges.push_back(it->edge);
			// 	it=it->twin->next;
			// }while(it!=t1);
			// new_edges.push_back(h1->next->twin->next->edge);
		}
	...
	for(auto ee:new_edges){
		if( (is_new(ee->halfedge->vertex) && !is_new(ee->halfedge->twin->vertex)) ||
		(!is_new(ee->halfedge->vertex) && is_new(ee->halfedge->twin->vertex))
		){
			nn++;
			auto e0 = ee->halfedge->vertex->position;
			auto e1 = ee->halfedge->twin->vertex->position;
			// std::cout<<e0.x<<e0.y<<e0.z<<std::endl;
			// std::cout<<e1.x<<e1.y<<e1.z<<std::endl;
			flip_edge(ee);
			e0 = ee->halfedge->vertex->position;
			e1 = ee->halfedge->twin->vertex->position;
			// std::cout<<e0.x<<e0.y<<e0.z<<std::endl;
			// std::cout<<e1.x<<e1.y<<e1.z<<std::endl;
	...
	// for(Halfedge_Mesh::EdgeRef eee= edges.begin(); eee!= edges.end(); eee++){
	// 	auto v1 = eee->halfedge->vertex->position;
	// 	auto v2 = eee->halfedge->twin->vertex->position;
...
void Halfedge_Mesh::isotropic_remesh(Isotropic_Remesh_Parameters const &params) {
	...
	// Optional! Only one of {A2Go1, A2Go2, A2Go3} is required!
	uint32_t outer_iterations = params.outer_iterations; //how many outer loops through the remeshing process to take
	float longer_factor = params.longer_factor; //edges longer than longer_factor * target_length are split
	float shorter_factor = params.shorter_factor; //edges shorter than shorter_factor * target_length are collapsed
	uint32_t smoothing_iterations = params.smoothing_iterations; //how many tangential smoothing iterations to run
	float smoothing_step = params.smoothing_step;
	// Compute the mean edge length. This will be the "target length".

	float mean_length = 0.0f;
	for(Halfedge_Mesh::EdgeRef e= edges.begin(); e != edges.end(); e++){
		mean_length+=e->length();
	}
	mean_length/=(edges.size()*1.0f);
	std::cout<<mean_length;
	...
	for(uint32_t i =0;i<outer_iterations;i++){
		...
		for(Halfedge_Mesh::EdgeRef e= edges.begin(); e != edges.end(); e++){
			if(e->length()>(mean_length*longer_factor)){
				split_edge(e);
			}
		}
		...
		for(Halfedge_Mesh::EdgeRef e= edges.begin(); e != edges.end(); e++){
			if(e->length()<(mean_length*shorter_factor)){
				collapse_edge(e);
			}
		}
		// -> Flip each edge if it improves vertex degree.
		for(Halfedge_Mesh::EdgeRef e= edges.begin(); e != edges.end(); e++){
			int vi = e->halfedge->vertex->degree();
			int vj = e->halfedge->twin->vertex->degree();
			int vk = e->halfedge->next->next->vertex->degree();
			int vl = e->halfedge->twin->next->next->vertex->degree();
			int before = std::abs(vi-6) +std::abs(vj-6) +std::abs(vk-6) +std::abs(vl-6) ;
			// after:
			vi-=1;
			vj-=1;
			vk+=1;
			vl+=1;
			int after = std::abs(vi-6) +std::abs(vj-6) +std::abs(vk-6) +std::abs(vl-6) ;
			if(after<before){
				flip_edge(e);
			}
		}
		...
		std::unordered_map< VertexRef, Vec3 > vertex_new_pos;
		for(uint32_t j =0;j<smoothing_iterations;j++){
			for(Halfedge_Mesh::VertexRef v= vertices.begin(); v != vertices.end(); v++){
				vertex_new_pos[v] = v->neighborhood_center();
			}

			for(Halfedge_Mesh::VertexRef ver= vertices.begin(); ver != vertices.end(); ver++){
				auto p =ver->position;
				auto q =  p+ smoothing_step*(vertex_new_pos[ver]-p);
				auto v = vertex_new_pos[ver]-p;
